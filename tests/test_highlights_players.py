...
def _cfg():
    return PlayerSigCfg(v_run=0.25, cluster_radius=0.12, cluster_min_players=4,
                        cluster_slow=0.08, cluster_min_dur_s=2.0,
...
def test_scattered_slow_players_no_signals():
    ...
    for fi in range(60):
        ...
        for pid, (x, y) in enumerate(spots):
            x2 = x + rng.normal(0, 2)  # ~2px/frame jitter << thresholds
            fr.append({"id": pid, "conf": 0.9, "box": [x2 - 10, y - 40, x2 + 10, y]})
        frames.append(fr)
    sig = player_signals(frames, fps, None, Space.PIXEL, _zones(), W, H, _Pitch(), _cfg(), bin_s=0.5)
