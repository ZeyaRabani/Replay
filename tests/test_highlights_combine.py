...
def _sig(n=400):
    ...
    return {"ball_attack_A": z.copy(), "ball_attack_B": z.copy(), "attack_A": z.copy(), "attack_B": z.copy(),
            ...
            "cluster_A": z.copy(), "cluster_B": z.copy(), "restart": z.copy()}
...
def test_goal_and_chance_and_merge():
    ...
    cfg = Config()
    sig = _sig()
    # goal on B at t=40 (bin 80): attack + audio + cluster + lost
    sig["ball_attack_B"][80] = 1.0
    sig["audio"][84:90] = 0.9
    sig["cluster_B"][82:88] = 1.0
    sig["ball_lost_B"][79] = 1.0
    # second anchor 5s later on B (t=45, bin 90) — should merge
    sig["ball_attack_B"][90] = 0.8
    # chance on A at t=120 (bin 240): attack + weak audio only
